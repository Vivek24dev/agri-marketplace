...
export default function Navbar() {
  ...
  return (
    <>
          ...
          <div className="flex justify-between items-center h-16">
            ...
            {user ? (
              <div className="flex items-center gap-3 sm:gap-4">
                {/* Choose City or Village button */}
                <button
                  onClick={() => setShowLocationModal(true)}
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-full bg-slate-100 hover:bg-emerald-50 text-slate-700 hover:text-emerald-800 border border-slate-200 hover:border-emerald-300 transition-all cursor-pointer shadow-2xs group"
                  title="Click to choose or change your City/Village"
                >
                  <MapPin className="w-3.5 h-3.5 text-emerald-600 group-hover:scale-110 transition-transform" />
                  <span className="max-w-[120px] sm:max-w-[160px] truncate">
                    {user.city_or_village || user.district || 'Select City/Village'}
                  </span>
                  <span className="text-[10px] font-extrabold text-emerald-700 bg-emerald-100/80 px-1.5 py-0.5 rounded-full">
                    Change
                  </span>
                </button>
                ...
                <button
                  ...
                  className="flex items-center gap-2 text-xs font-semibold px-3.5 py-2 rounded-lg text-slate-600 hover:text-red-600 hover:bg-red-50 border border-slate-200 hover:border-red-200 transition-colors cursor-pointer"
      ...
      {/* City or Village Selection Modal */}
