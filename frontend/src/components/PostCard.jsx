...
export default function PostCard({ post, isOwner, onDealingDone }) {
  ...
  const defaultMessage = isProduce
    ? `Hello ${post.user_name || 'Farmer'}, I saw your listing for "${post.title}" (${post.quantity}kg ${post.crop_type} at ₹${post.price_per_unit}/kg) on Agro-Market Beta. I am interested in purchasing.`
    : `Hello ${post.user_name || 'Buyer'}, I saw your requirement for "${post.title}" (${post.quantity}kg ${post.crop_type} at ₹${post.price_per_unit}/kg) on Agro-Market Beta. I have stock available and can supply.`;
